...
- 요청된 결과까지 구현하고 관련 검증을 마친다. 통상적인 구현·리팩터링 선택은 자율적으로 결정한다.
- 결과를 크게 바꾸는 입력이 부족할 때 질문하고, 답을 기다리는 동안 독립적인 작업은 진행한다.
- 기존 사용자 변경을 보존한다. 현재 요청이나 대화에서 승인한 일을 과거 문서 때문에 재승인받지 않는다.
...
- MCP는 localhost와 기본 Bearer 인증을 유지하며 `/health`만 공개한다.
  ...
  외부 CLI 설정 생성·제거는 Settings의 명시적 요청으로만 수행한다.
