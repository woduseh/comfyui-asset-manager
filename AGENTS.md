...
- 검토 요청에는 발견 사항과 수정안을 제시하고, 변경 요청은 구현과 관련 검증까지 완료한다.
- 관련 코드·테스트를 근거로 통상적인 구현·도구·검증을 자율적으로 선택한다. 선택적 선호는 가정을 밝혀 진행하고,
  문맥으로 판단할 수 없는 필수 입력은 질문한다. 답변이 필요한 작업만 보류하고 독립 작업은 계속한다.
- 기존 사용자 변경을 보존한다. 파괴적 작업·외부 쓰기·비용 지출·실질적인 범위 변경에는 사용자 요청이나 승인이 필요하다.
  같은 범위의 승인은 재사용하되, 별도의 실행 직전 확인이 명시된 경우에는 따른다. 침묵은 승인이 아니다.
...
- 코드 변경은 관련 테스트와 타입 검사를 실행하고, 영향 범위가 넓거나 빌드·패키징을 건드리면 전체 검증으로 넓힌다.
  DB 저장·경로 접근·IPC 검증·큐 상태 변경은 실패 경로와 데이터 보존을 확인한다.
  단순 추출을 이유로 구현을 복제한 테스트를 추가하거나, 통과한 검증을 새 근거 없이 반복하지 않는다.
...
- 전체 검증은 `npm run verify`, CI와 같은 커버리지 게이트는 `npm run verify:coverage`다.
  보고서 경로를 공유하므로 같은 체크아웃에서 통합 검증을 동시에 실행하지 않는다.
- 앱 동작 확인은 격리 검사 `npm run smoke`, 환경 문제는 `npm run doctor`를 사용한다.
- 전체 검증·앱 검사·환경 진단을 실행하거나 결과를 판정할 때는
  [개발과 검증 루프](docs/development.md)의 범위·전제·보고서 설명을 참고한다.
  실제 결과와 미검증 영역을 보고하며 과거 성공을 이번 검증 결과로 사용하지 않는다.
