...
- **MCP 서버**: LLM CLI (Copilot, Claude, Gemini, Codex)가 앱 기능을 도구로 호출 가능 (27개 도구)
- **대량 작업 도구**: 일괄 생성/업데이트, 파일 가져오기 (JSON/CSV/MD), 모듈 복제, 태그 치환
- **멀티 CLI 자동 설정**: 터미널 탭 생성 시 MCP 서버 자동 시작
...
3. 연결 실패 시 **설정** 페이지에서 호스트·포트를 변경합니다
...
1. **기본 설정**: 워크플로우, 조합당 생성 수, 시드 모드
2. **모듈 & 프롬프트**: 조합 모듈 매트릭스 (왼쪽) + 슬롯별 모듈 매핑 (오른쪽)
3. **확인**: 총 이미지 수 미리보기 → **생성**

작업 카드의 **시작** 버튼으로 실행 → 상단 상태 바에서 진행률 확인
...
- 경로 순회 차단, IPC 입력 검증, Repository 필드 화이트리스트
