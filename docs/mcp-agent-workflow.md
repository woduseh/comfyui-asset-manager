...
MCP에 연결된 에이전트에게 캐릭터 프로필과 필요한 감정을 전달하면 워크플로우 확인, 프롬프트 모듈 작성, 배치 미리보기, 생성, 이미지 검토까지 이어서 맡길 수 있어요. 서버 시작 안내는 핵심 작업 계약을 제공하고, `get_generation_guide`는 작업에 필요한 설명을 선택해서 읽는 데 사용해요. MCP 프롬프트 템플릿을 별도로 실행할 필요는 없어요.

`get_generation_guide`에 `{}` 또는 `{"topic":"overview"}`를 보내면 짧은 개요와 주제별 사용 조건(`topics`)을 반환해요. 상세 단계(`steps`)와 해당 예제(`example`)는 `workflow`(워크플로우 선택·준비), `prompts`(모듈·프롬프트·variant), `execution`(미리보기·저장·실행·대기), `review`(시각 검토·허용된 추가 개선), `recovery`(실패 진단·실행 제어) 중 필요한 `topic`을 지정해서 조회해요. 예를 들어 실행 계약은 `{"topic":"execution"}`으로 읽어요. 모든 주제를 미리 읽거나 적합한 기존 워크플로우를 다시 준비할 필요는 없어요.
...
캐릭터 1개 × 의상 1개 × 감정 3개 × 조합당 1회이면 총 3회 실행이에요. 이 모듈 구성은 예시이며, 요청한 외형·의상·감정과 수량을 유지하도록 항목을 선택해요. 워크플로우의 `batch_size`와 이미지 출력 노드에 따라 한 번에 여러 이미지가 나올 수 있으므로, 정확히 3장이 필요하면 워크플로우도 1회당 1장을 출력하도록 확인해요. 특정 배치에서 무관한 항목을 제외하려면 `selectedItemIds`를 명시해요. `enabled`는 재사용되는 항목 자체의 활성 상태를 바꿀 때 사용해요.
...
Danbooru 검사는 태그 프롬프트에만 사용해요. 새로 작성하거나 변경한 태그를 저장 전에 `validate_danbooru_tags`로 한 번에 최대 200개씩 검증하고, 바뀌지 않은 태그의 성공 결과는 새 근거가 없는 한 재사용해요. 조회 불가로 `unverified`인 태그는 잘못된 태그로 단정하거나 계속 재시도하지 않고 확인 한계를 알려요. `danbooru_tag_guide`는 서식·검증 안내만 제공하며 인기 태그 예시는 필요할 때 `get_popular_danbooru_tags`로 조회해요.

`validate_module_tags`는 기본적으로 `tags` variant를 검사하며, 기본 프롬프트가 자연어이면 `include_default: false`를 지정해요. 다른 태그 variant는 `variant_names`로 선택할 수 있어요. 자연어를 태그 검증 결과에 맞춰 무조건 바꾸지 않아요.
...
| 시나리오                                | 성공 기준                                                                                      |
| --------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 적합한 기존 워크플로우로 생성           | 필요한 가이드 주제만 조회하고 신규 워크플로우 준비를 불필요하게 반복하지 않음                  |
