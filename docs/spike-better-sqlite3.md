...
> 2026-07-04 환경에서 수행한 실험 기록이에요. 버전·성능·파일 크기·테스트 수와 전환 보류 결정은 당시 결과이며 현재 성능이나 최신 버전을 나타내지 않아요. 재검토 조건은 당시 제안으로, 현재 작업을 자동 실행하거나 전환을 금지하는 규칙이 아니에요. 이후 sql.js 최적화 측정은 [2026-09-05 DB 성능 보고서](performance/2026-09-05-database.md)에서 확인할 수 있어요.
...
**당시 결정은 전환 보류였어요.** 기존 DB 호환성과 Electron 재빌드는 확인했지만, 합의한 성능
...
- 후보 버전: `better-sqlite3@12.11.1` (조사 당시 `npm view` 기준 최신)
...
당시에는 다음 중 하나가 확인될 때 재측정하는 방안을 제안했어요.
...
후속 측정 후보에는 GitHub Actions `windows-latest`의 clean install·패키징 시간과 실제 설치
프로그램 시작이 포함됐어요. 이 기록에는 해당 측정 결과가 없어요.
