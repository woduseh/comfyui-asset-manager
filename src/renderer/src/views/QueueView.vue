...
const jobColumns: DataTableColumns = [
  { title: t('common.name'), key: 'name', width: 180 },
  {
    ...
    render(row) {
      ...
      return h(
        NTag,
        {
          type: (colors[row.status as string] || 'default') as
            | 'success'
            | 'info'
            | 'warning'
            | 'error'
      ...
      return h(
        ...
        { align: 'center' },
        {
          default: () => [
            h(
              NProgress,
              { type: 'line', percentage: pct, style: 'width: 120px;', showIndicator: false },
              {}
            ),
  ...
  {
    title: t('batch.column.failed'),
    key: 'failed_tasks',
    width: 60,
    render(row) {
      const failed = (row.failed_tasks as number) || 0
      return failed > 0
        ? h(NTag, { type: 'error', size: 'tiny' }, { default: () => String(failed) })
        : '-'
    }
  },
  {
    ...
    render(row) {
      ...
      if (status === 'running') {
        buttons.push(
          h(
            NButton,
            { size: 'tiny', type: 'warning', onClick: () => handlePause() },
            {
        ...
        buttons.push(
          h(
            NButton,
            { size: 'tiny', type: 'error', onClick: () => handleCancel() },
            {
      ...
      if (status === 'paused') {
        buttons.push(
          h(
            NButton,
            { size: 'tiny', type: 'primary', onClick: () => handleResume() },
            {
  ...
  <div class="queue-view">
    <div style="display: flex; justify-content: space-between; align-items: center">
      ...
      <NSpace>
        ...
        <NButton size="small" @click="loadAllJobs">{{ t('queue.refresh') }}</NButton>
      </NSpace>
    </div>

    <!-- Active processing summary -->
    <template v-if="queueStore.activeJobs.length > 0">
      <NCard style="margin-top: 16px" :title="t('queue.currentRunning')">
        <NGrid :cols="4" :x-gap="16">
          <NGridItem>
            <NStatistic :label="t('queue.totalProgress')" :value="queueStore.totalProgress + '%'" />
          </NGridItem>
